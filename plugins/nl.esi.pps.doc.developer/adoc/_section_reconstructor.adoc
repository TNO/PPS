...
----
<1> Extending `nl.esi.pps.common.emf.ecore.resource.LineNumberResource<State>` allows you to implement the `doLoadLine(String, int, State)` method.
...
<4> The TMSC model instance is _well-formed_ by calling the `postReconstruct()` method on the `TmscReconstructor`.
    ...
    Finally, the reconstructed TMSC, its architecture and optionally the traced metrics are added to the resource contents.