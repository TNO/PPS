...
public class WalkinshawLocalGLTSScorer<S, T, U extends GLTS<S, T>> extends WalkinshawScorer<S, T, U> {
    ...
    public void adjustNrOfRefinements(int newNrOfRefinements) {
        this.nrOfRefinements = newNrOfRefinements;
    }
...
//                true);
        //Added: nrOfRefinements can now be selected wrt. the graph size (increases the result quality for small graphs)
        if(lhs.getStates().size() <= 45 && rhs.getStates().size() <= 45)
            adjustNrOfRefinements(5);
        else
            adjustNrOfRefinements(1);
        return computeScores(lhs, rhs, (glts, state) -> glts.getOutgoingTransitions(state), Transition::getTarget,
                true);
    }
...
//                false);
            ...
            Function<Transition<S, T>, State<S>> stateSelector, boolean isForward)
    {
        ...
        //Added: Find already-matched state pairs and assign them score 1.0, -1.0 otherwise
        System.err.println("Enter scoring");
        long start = System.nanoTime();

        Map<State<S>, State<S>> matchedStates = new HashMap<>();
        for (State<S> leftState: lhs.getStates()) {
            for (State<S> rightState: rhs.getStates()) {
                scores.setEntry(leftState.getId(), rightState.getId(), -1d);
                if(statePropertyCombiner.areAlreadyMatched(leftState.getProperty(), rightState.getProperty())) {
                    matchedStates.put(leftState, rightState);
                    scores.setEntry(leftState.getId(), rightState.getId(), 1d);
                }
            }
        }

        System.err.println("Enter phase 2 of scoring");
        ...
        for (int i = 0; i < nrOfRefinements; i++) {
            ...
            for (State<S> leftState: lhs.getStates().stream().filter(s -> !matchedStates.containsKey(s)).collect(Collectors.toList())) {
                for (State<S> rightState: rhs.getStates().stream().filter(s -> !matchedStates.containsValue(s)).collect(Collectors.toList())) {
        ...
        System.err.println("Exit scoring - "+(end-start)/1000000 + " ms\n");
    ...
    private double similarityScore(U lhs, U rhs, State<S> leftState, State<S> rightState, RealMatrix scores,
            ...
            Function<Transition<S, T>, State<S>> stateSelector, boolean isForward)
    {
        // Added: Stop matching if 'leftState' and 'rightState' are uncombinable, the similarity score is then set to -1.0.
