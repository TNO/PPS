...
class DependencyTopologicalOrder implements TopologicalOrderIterator<Dependency> {
    ...
    for (final Dependency element : _dependencies) {
      {
        ...
        Event _source = element.getSource();
        boolean _tripleEquals = (_source == null);
        if (_tripleEquals) {
          _xifexpression = 0;
        } else {
          _xifexpression = this.tmsc.getIncomingDependencies(element.getSource()).size();
        }
  ...
  private LinkedList<Dependency> doConsume(final Dependency dependency) {
    ...
    if (_tripleNotEquals) {
      Collection<Dependency> _outgoingDependencies = this.tmsc.getOutgoingDependencies(dependency.getTarget());
      for (final Dependency candidate : _outgoingDependencies) {
        {
