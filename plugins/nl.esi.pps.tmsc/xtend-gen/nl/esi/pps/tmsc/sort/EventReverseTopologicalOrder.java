...
class EventReverseTopologicalOrder implements TopologicalOrderIterator<Event> {
  @Extension
  ...
  public EventReverseTopologicalOrder(final ITMSC tmsc) {
    this.tmsc = tmsc;
    ...
    for (final Event element : _events) {
      {
        final Function1<Dependency, Boolean> _function = (Dependency it) -> {
          ...
          return Boolean.valueOf((_target == null));
        };
        final int threshold = IterableExtensions.size(IterableExtensions.<Dependency>reject(this.tmsc.getOutgoingDependencies(element), _function));
        if ((threshold == 0)) {
  ...
  private LinkedList<Event> doConsume(final Event event) {
    ...
    Iterable<Event> _filterNull = IterableExtensions.<Event>filterNull(IterableExtensions.<Dependency, Event>map(this.tmsc.getIncomingDependencies(event), _function));
