...
package class DependencyReverseTopologicalOrder implements TopologicalOrderIterator<Dependency> {
    ...
    new (ITMSC tmsc) {
        ...
        for (element : tmsc.dependencies) {
            val threshold = element.target === null ? 0 : element.target.outgoingDependencies.size
            if (threshold == 0) {
    ...
    def private doConsume(Dependency dependency) {
        ...
        if (dependency.source !== null) {
            for (candidate : dependency.source.incomingDependencies) {
