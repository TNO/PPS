...
package class EventReverseTopologicalOrder implements TopologicalOrderIterator<Event> {
    ...
    new (ITMSC tmsc) {
        ...
        for (element : tmsc.events) {
            val threshold = element.outgoingDependencies.reject[target === null].size
            if (threshold == 0) {
    ...
    def private doConsume(Event event) {
        ...
        for (candidate : event.incomingDependencies.map[source].filterNull) {
